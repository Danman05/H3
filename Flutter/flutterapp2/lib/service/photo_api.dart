...
class ApiService {
  ...
  // Fetches a list of images (as base64 strings) from the API
   Future<List<String>> getAllImages() async {
        List<String> result = [];
    try {
      HttpClientRequest request = await httpClient.getUrl(Uri.parse("$_baseUrl/image/all"));
      ...
      if (response.statusCode == 200) {
        ...
        print('Failed to get all images: $responseBody ${response.statusCode}');
              }
    } catch (e) {
  ...
  Future<String> addImage(String base64string) async {
    try {
      ...
      final request = await httpClient
          .postUrl(Uri.parse("$_baseUrl/image/add"));

      // Add headers for content-type.
      request.headers.set(HttpHeaders.authorizationHeader, 'Bearer ${_flutterSecureStorage.read(key: 'Authorization')}');
      ...
      else if(response.statusCode == HttpStatus.forbidden || response.statusCode == HttpStatus.unauthorized) {
  ...
  Future<String> login(Credentials credentials) async {
    try {
       // Construct the POST request
      final HttpClientRequest request = await httpClient.postUrl(Uri.parse("$_baseUrl/auth"));
      ...
      // Read the response.
      if (response.statusCode == HttpStatus.ok) {
      _flutterSecureStorage.write(key: 'Authorization', value: '123');
        return "";
